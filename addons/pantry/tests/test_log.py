...
def test_formatter_renders_a_string_message() -> None:
    formatter = log.JSONFormatter("joshua-pantry")
    line = formatter.format(_make_record("resolved item"))
    data = json.loads(line)
    assert data["message"] == "resolved item"
    assert data["service"] == "joshua-pantry"
    assert data["level"] == "INFO"
    assert data["logger"] == "pantry.test"
    assert "ts" in data


def test_formatter_flattens_a_dict_message() -> None:
    formatter = log.JSONFormatter("joshua-pantry")
    line = formatter.format(_make_record({"message": "purchase recorded", "item_count": 3}))
    data = json.loads(line)
    assert data["message"] == "purchase recorded"
...
def test_get_logger_returns_a_named_logger() -> None:
    assert log.get_logger("pantry").name == "pantry"
