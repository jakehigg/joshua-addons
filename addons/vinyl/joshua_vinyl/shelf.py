...
(compilation, soundtrack) goes to the first configured section that lists
that trait. Those sections sort after Z, in configured order. A sort-name
that starts with a digit or a symbol files under ``#``, before A.
"""
...
def traits_of(artist: str, styles: list[str], formats: list[dict[str, Any]]) -> set[str]:
    """The special traits of a release: ``compilation`` and ``soundtrack``.

    A Discogs compilation is credited to ``Various`` or carries the
    ``Compilation`` format description. A soundtrack carries the
    ``Soundtrack`` or ``Score`` style.
    """
    traits: set[str] = set()
    descriptions = {
        str(item).casefold() for fmt in formats for item in (fmt.get("descriptions") or [])
    }
    if artist.strip().casefold() in VARIOUS_ARTISTS or "compilation" in descriptions:
